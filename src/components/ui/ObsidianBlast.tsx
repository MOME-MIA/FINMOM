...
export interface ObsidianBlastRef {
    triggerShockwave: (color: string) => void;
    triggerWarp: () => void;
    triggerPulse: (color?: string) => void;
}
...
export const ObsidianBlast = forwardRef<ObsidianBlastRef, ObsidianBlastProps>(({
    ...
    patternScale = 3,
    patternDensity = 1.2,
    liquid = false, // Disable Liquid by default as requested
    liquidStrength = 0.12,
    liquidRadius = 1.2,
    pixelSizeJitter = 0.5,
    enableRipples = true,
    ...
    const flashRef = useRef({ r: 0, g: 0, b: 0, strength: 0 });
    const warpRef = useRef({ active: false, strength: 0 });
    ...
    useImperativeHandle(ref, () => ({
        triggerShockwave: (hexColor: string) => {
            if (!threeRef.current) return;
            const c = new THREE.Color(hexColor);
            flashRef.current = { r: c.r, g: c.g, b: c.b, strength: 1.0 };

            // Trigger a massive ripple from center
            const t = threeRef.current;
            const ix = t.clickIx;
            // Center of screen roughly
            t.uniforms.uClickPos.value[ix].set(t.renderer.domElement.width / 2, t.renderer.domElement.height / 2);
            t.uniforms.uClickTimes.value[ix] = t.uniforms.uTime.value;
            t.clickIx = (ix + 1) % MAX_CLICKS;
        },
        triggerWarp: () => {
            warpRef.current.active = true;
            warpRef.current.strength = 0;
        },
        triggerPulse: (hexColor: string = "#ffffff") => {
            if (!threeRef.current) return;
            const c = new THREE.Color(hexColor);
            // Subtle flash
            flashRef.current = { r: c.r, g: c.g, b: c.b, strength: 0.3 };
        }
    }));
    ...
    useEffect(() => {
        ...
        if (mustReinit) {
            ...
            // Force pointer-events: none on the canvas to prevent blocking
            ...
            const animate = () => {
                ...
                // Handle Flash Decay
                if (flashRef.current.strength > 0) {
                    flashRef.current.strength -= 0.02; // Slower decay for visibility
                    if (flashRef.current.strength < 0) flashRef.current.strength = 0;
                    uniforms.uFlashColor.value.setRGB(flashRef.current.r, flashRef.current.g, flashRef.current.b);
                    uniforms.uFlashStrength.value = flashRef.current.strength;
                }

                // Handle Warp
                if (warpRef.current.active) {
                    warpRef.current.strength += 0.02;
                    speedRef.current = speed + warpRef.current.strength * 2.0;
                    uniforms.uScale.value = patternScale - warpRef.current.strength * 0.5; // Zoom in
                } else {
                    speedRef.current = speed;
    ...
    return (
        <div
            ...
            style={style}
            aria-label="Obsidian Blast interactive background"
        />
    );
});
